...
class Display():
    ...
    def refresh(self):
        ...
        if time.time()-self.game.time > (0.99)*TMAX: #cuando se haya llegado a un tiempo fijado
            ...
            for i in range(n_naves):
                if (score[i]<score[mejor]):
                    mejor=i
            text2=font2.render(f"Ganador: Player {mejor+1}", 1, WHITE)
            self.background = pygame.image.load('negro.jpeg')
            self.background = pygame.transform.scale(self.background,(1280,720))
            self.screen.blit(text1,(300, 250))
            self.screen.blit(text2,(300,400))
